...
import com.example.yolov5tfliteandroid.utils.BoundingBox;
import com.example.yolov5tfliteandroid.utils.NMS;
...
public class Yolov5TFLiteDetector {
    ...
    private final int[] OUTPUT_SIZE = new int[]{1, 5, 8400};
    private Boolean IS_INT8 = false;
    private final float DETECT_THRESHOLD = 0.25f;
    private final float IOU_THRESHOLD = 0.2f;
    private final float IOU_CLASS_DUPLICATED_THRESHOLD = 0.7f;
    private final String MODEL_YOLOV5S = "box_train_lan_1_float32.tflite";
//    private final String MODEL_YOLOV5S = "yolov5s-dynamic.tflite";
    ...
    private final String MODEL_YOLOV5S_INT8 = "box_train_lan_1_full_integer_quant.tflite";
    ...
    public ArrayList<BoundingBox> detect(Bitmap bitmap) {
        ...
        for (int i = 0; i < OUTPUT_SIZE[2]; i++) {
            float dw = recognitionArray[OUTPUT_SIZE[2]*2 + i] / 2;
            float dh = recognitionArray[OUTPUT_SIZE[2]*3 + i] / 2;
            int x = (int) recognitionArray[i];
            int y = (int) recognitionArray[OUTPUT_SIZE[2] + i];
            int xmin = Math.max(0, x - (int) dw);
            int ymin = Math.max(0, y - (int) dh);
            int xmax = Math.min(INPNUT_SIZE.getWidth(), x + (int) dw);
            int ymax = Math.min(INPNUT_SIZE.getHeight(), y + (int) dh);
            float confidence = recognitionArray[OUTPUT_SIZE[2]*4 + i];
            if (confidence > DETECT_THRESHOLD){
                boxes.add(new BoundingBox(xmin, ymin, xmax, ymax, confidence));
            }
        }
        ArrayList<BoundingBox> selectedBoxes = NMS.applyNMS(boxes, IOU_THRESHOLD);
